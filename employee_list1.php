...
if (isset($_SESSION['user_id'])) {
    ...
    if ($result && $result->num_rows > 0) {
        ...
        $subordinateQuery = "SELECT Employee.EmployeeID, Employee.Name, JobRole.JobRole AS JobRoleName
                        FROM Employee
                        JOIN JobRole ON Employee.JobRoleID = JobRole.JobRoleID
...
function canEvaluate($employeeID, $conn) {
    ...
    // Print debug information for the current date, year, and trimester
    echo "Debug - Current Year: $currentYear, Current Trimester: $currentTrimester, Is Last Week: " . ($isLastWeek ? 'Yes' : 'No') . "<br>";

    // Check if the employee has already been evaluated in the current trimester
    $checkQuery = "
        SELECT COUNT(*) as evaluationCount
        FROM EmployeeEvaluation 
        WHERE EmployeeID = $employeeID 
          AND Year = $currentYear
          AND Trimester = $currentTrimester
    ";
    ...
    // Print debug information for evaluation count
    echo "Debug - Evaluation Count for Employee $employeeID: " . $evaluationData['evaluationCount'] . "<br>";

    // Conditions to allow evaluation: last week of the trimester and no prior evaluation in this trimester
    return $isLastWeek && $evaluationData['evaluationCount'] == 0;
}*/
...
<div class="container">
    ...
    <main class="main-content">
        <header class="header">
            <div class="user-info">
                ...
                <img src="<?php echo htmlspecialchars($userPhoto); ?>" alt="User Photo" class="user-photo" style="width:60px; height:60px; border-radius:50%;">
    ...
    <div class="employee-card">
        <div class="employee-info">
            ...
            <p><strong><?php echo htmlspecialchars($subordinate['Name']); ?></strong></p>
            <p><?php echo htmlspecialchars($subordinate['JobRoleName']); ?></p>
        </div>
        
        <?php if ($trimesterToEvaluate): ?>
            <!-- If evaluation is allowed, set the link with the correct trimester and year -->
            <a href="evaluate_employee.php?employeeID=<?php echo $subordinate['EmployeeID']; ?>&trimester=<?php echo $trimesterToEvaluate; ?>&year=<?php echo $currentYear; ?>" class="save-changes-btn">Evaluate</a>
        <?php else: ?>
